const {
  request, app, authHeader, registerRequester, registerWorker, createAdmin, makeBookingAt, closePool,
} = require('./helpers');
...
describe('book from worker profile', () => {
  test('POST /bookings/book/:workerId auto-creates a task and a valid pending booking', async () => {
    ...
    const task = await request(app).get(`/api/tasks/${res.body.task_id}`).set(authHeader(requester.token));
...
describe('in-app notifications', () => {
  test('a lifecycle transition notifies the other party; owner can list + mark read', async () => {
    const { requester, worker } = await makeBookingAt('checkedIn');
